...
public class Cylinder : Shape
{
    ...
    void Start()
    {
        ...
        info.surfaceAreaCalculation += $" = {(2.0f * Mathf.PI * info.lengths[1].value * info.lengths[0].value) + (2 * Mathf.PI * Mathf.Pow(info.lengths[1].value, 2))}cm^2.";

        info.volumeCalculation = "The volume of a cylinder is \u03C0 x Radius^2 x Height.";
        info.volumeCalculation += $"\n\nIn this case that would be \u03C0 x {info.lengths[1].value}^2 x {info.lengths[0].value} = {Mathf.PI * Mathf.Pow(info.lengths[1].value, 2) * info.lengths[0].value}^3.";
    }
